...
const listWorkoutsAPIHandler = async (req: ListWorkoutsReq, res: Res<ListWorkoutsRes>): Promise<void> => {
    try {
...
export const listWorkoutsAPI = withMiddleware(
    verifyQueryId<['owner']>(['owner']),
    authRateLimit(checkAuth()),
    listWorkoutsAPIHandler,
);
